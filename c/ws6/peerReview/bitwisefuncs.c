...
long Pow2(unsigned int x, unsigned int y) /*CHECKED*/
{
	return x << y;
}

enum status IsItpow2(unsigned n) /*CHECKED 1 ERROR*/
{
	int i = 0;
	long res = 2;
	for (i = 0 ; i < n ; i ++)
...
enum status IsItpow2p2(unsigned n) /* CHECKED */
{
	if ( 0 == (n & (n-1)))
	{
		return true;
...
void ThreeOn(unsigned int *arr) /*CHECKED*/
{
	unsigned int temp = 0; 
	int counter = 0;
	int i = 0;
...
unsigned int ByteMirror(unsigned int x) /*CHECKED*/
{
	int m = 1;
...
unsigned char BothTwoAndSix(unsigned char x) /*CHECKED*/
{
	return ( ( 1 & (x>>1)) && ( 1 & (x>>5)) );
}
