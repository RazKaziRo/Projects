...
void TestSchedulerRemoveItsSelf()
{
	...
	newUid = SchedulerAddTask(newScheduler, &Remove, 4, &NewRemove);
...
void TestSchedulerRemoveTask()
{
	...
	SchedulerAddTask(newScheduler, &Remove, 10, &NewRemove);
	SchedulerAddTask(newScheduler, &StopScheduler,6, newScheduler);

	RUN_TEST(4 == SchedulerSize(newScheduler), "FAIL: WRONG CREATE SIZE (0)");
