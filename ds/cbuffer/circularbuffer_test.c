...
#include "cbuffer.h" /*typedef CBuffer*/
...
int main()
{
	...
	CBufferRead(char_buffer,cbuffer, 3); /* [-,-,-,1,2,3,1,2,3,4] */
	TEST(3 == cbuffer->read_index, "Read()");
	CBufferWrite(cbuffer, char_test, 2); /* [1,2,-,1,2,3,1,2,3,4] */
	...
	CBufferRead(char_buffer,cbuffer, 8); /* [8,9,-,-,-,-,-,-,-,-] */
	TEST(10 == cbuffer->read_index, "Read(), exactly to capacity");
	CBufferRead(char_buffer,cbuffer, 2); /* [-,-,-,-,-,-,-,-,-,-] */
