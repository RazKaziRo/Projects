...
static size_t HashGetManipulateKey(const hash_t *hash, const void *data)
{
...
{	
	...
	key = HashGetManipulateKey(hash, data);
...
{
	size_t key = HashGetManipulateKey(hash, data);
...
int HashForeach(hash_t *hash, action_func_t action, void *param)
{
	...
	{	
		...
		res = DLLForEach(dll_start, dll_end, action, param);
